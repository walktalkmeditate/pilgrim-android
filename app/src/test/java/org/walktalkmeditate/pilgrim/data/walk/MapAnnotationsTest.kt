...
class MapAnnotationsTest {
    ...
    @Test fun emptySamples_returnsEmpty() {
        assertTrue(computeWalkMapAnnotations(emptyList(), emptyList()).isEmpty())
    }
    ...
    // No voiceRecording_pinAtClosestSampleToStart test: user product
    // decision 2026-08-18 removed voice-recording pins from the summary
    // map entirely, and [computeWalkMapAnnotations] no longer accepts a
    // `voiceRecordings` parameter at all — see
    // [WalkSummaryViewModelTest.walkAnnotations_populated_excludesVoiceRecording]
    // for the pipeline-level proof that a walk's voice recordings never
    // reach the map as an annotation.
