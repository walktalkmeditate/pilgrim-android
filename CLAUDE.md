...
**Parity target: pilgrim-ios main @ `e7051bc`** (2026-08-29). This advanced from `0172e2b` (the shipped `v1.11.0` tag) when the pre-release fold-in stage ported iOS PRs #72 (oblique voice + prompt-relevance fixes) and #74 (theme-layer noise) into Phase 20 before the v1.5.0 tag — the R2 rule exercised as designed. Anything iOS shipped AT OR BEFORE `e7051bc` is in-scope for Android port. Anything iOS ships AFTER `e7051bc` is OUT OF SCOPE — with one bounded exception: deltas landing before Android v1.5.0 ships are re-diffed and triaged per the fold-in rule (R2 in `docs/brainstorms/2026-08-25-ios-v1110-parity-retarget-requirements.md`): chores, hotfixes, and incremental refinements to the dossier/senses/chips surfaces fold into Phase 20; new headline features, reverts of ported work, or redesigns require explicit user re-triage.

To diff iOS for in-scope work that hasn't landed on Android yet:
```bash
cd ../pilgrim-ios && git log --oneline e7051bc
```

Comparing to a future iOS HEAD past `e7051bc` is fine for context, but parity work targets `e7051bc` only.
