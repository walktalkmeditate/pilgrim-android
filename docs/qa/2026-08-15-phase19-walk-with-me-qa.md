...
8. **Toggle race (tracked follow-up from U8 re-review):** rapid Interactive off→on double-taps ×10 against the real transcoder → no row stuck "audio removed", and a subsequent share's page voice count equals the included count (worst endpoint: silent drop). If reproduced: apply the pre-specified fix (serialize prep/cleanup via field-held Job `cancelAndJoin` or per-VM Mutex) before release.
