...
- [ ] **Version-bump re-sweep** (fold-in `ANALYSIS_VERSION` 1→2): installing this build over an EARLIER Phase 20 debug build (one that already wrote v1 contexts) must re-arm the backfill and re-analyze everything — stale-version contexts are pruned, not trusted. Watch `adb logcat` (tag `ThreadsBackfill`) for the `sweep starting` line followed by a full-pass outcome line — a sweep that never ran logs neither, and a sweep that bailed says which gate stopped it. Then read a freshly written context file directly: `adb exec-out run-as org.walktalkmeditate.pilgrim.debug cat files/transcript_contexts/<uuid>.json.gz | gunzip` (take a `<uuid>` from the `ls` in section D) — it must visibly carry `"analysisVersion":2`. That is the cheapest confirmation the re-sweep actually ran; a file missing the key entirely is a pre-fold-in write, which now reads as stale rather than as current. Finally confirm themes that previously named filler/`time`/`person`/`app` are gone. A device with no prior Phase 20 build has nothing to re-sweep — note which case you tested.
