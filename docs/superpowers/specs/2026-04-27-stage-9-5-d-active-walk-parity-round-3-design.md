...
- **Pre-walk intention typed + Saved, then user navigates away (back-out before Start):** Walk never started; `preWalkIntention` is set but Walk row was never inserted; `rememberSaveable` clears when the user navigates away from ActiveWalkScreen (NavBackStackEntry disposes the local state). **Justification for the asymmetry vs iOS:** iOS holds intention drafts via `viewModel.intention` which lives on the ActiveWalkViewModel — also tied to the screen's lifetime. iOS's auto-prompt-on-arrival re-shows the dialog on return so the loss is invisible. Android's explicit-tap pill makes the loss visible: a back-out + return shows "Set an intention" again. This is the deliberate trade for Pilgrim's "no-friction" framing — the user's act of leaving the surface communicates "I'm not committed to that intention." Document this as expected UX rather than "draft loss."
- **Process death between Save and Start:** `rememberSaveable` does NOT survive process death; user retypes on next entry. Acceptable — same UX as iOS Cancel-then-process-die. NOTE: a related minor weirdness — `showPreWalkIntention: rememberSaveable<Boolean>` DOES survive into the Bundle, so a user who had the dialog open at process-death time will see the dialog reopen on restart with an empty text field (not their draft, since the text isn't saved in the Bundle). Acceptable: the empty field implies "the thing was open, type something." If we wanted full draft persistence we'd add DataStore (deferred per Non-goals).
