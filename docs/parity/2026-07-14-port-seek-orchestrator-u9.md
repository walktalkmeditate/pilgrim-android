...
**Pre-departure reroll (D8 — SUPERSEDED 2026-07-15):** the original U9 divergence read: on Android the engine boots at walk start, so a pre-departure "Seek anew" has no engine and the orchestrator silently regenerates the **pending session's** chain (`regeneratingRemainder(0, lastKnownLocation, …)` → `sessionStore.set(copy(chain =…))`) — no pulse possible. Device QA showed the walk-start boot itself was the drift: iOS's `beginSeekGPSLock` → `startSeekEngine` is stage-independent and boots the engine on the first ≤50 m fix, pre-departure, so fog/crescent/sonar are checkable before stepping off (iOS 85373c1). The orchestrator now boots on the STAGED session (`combine(walkState, pending)`) and the pre-departure reroll rides the live `engine.seekAnew` exactly like iOS — immediate stale-distance pulse included. `rerollPendingSession` is deleted; the staging-to-boot scheduling gap leaves `seekAnewRequested` a no-op, matching iOS's `guard let engine`.
...
- Section placed after the intention/waypoint rows, before Traces (iOS order), self-gated on an active seek session. Gate: `enginePhase != null || pendingSession != null` — iOS's `seekEngine != nil` covers pre-departure because its engine boots at the gateway; Android's pre-departure session is the pending chain (D8), so the gate ORs both. Wander renders nothing. *(2026-07-15, D3 revision: pre-departure both sides of the OR are now non-null — the engine boots on staging and the pending stays staged until adoption; the gate is unchanged and still correct.)*
...
| D3 | Engine boots at GPS lock inside setup; chain never leaves the VM | ~~Boot at walk start on (Active + mode Seek + pending session), consuming `SeekSessionStore.pending`~~ **Revised 2026-07-15** (device QA — no fog/crescent/pings on the ready screen): boot on the STAGED session via `combine(walkState, pending)` — pre-departure, before any walk, matching iOS's stage-independent GPS-lock boot. The Active transition ADOPTS the running engine (`sessionWalkId` set, pending consumed at adoption); a pending predating a `Finished.endedAt` never boots. The restore filter is unchanged in substance: the in-memory store is empty after process death, so restored walks can never reboot | U8/U9 split; consumption-at-adoption keeps every existing pending-clear path (back-out `onCleared`, setup cancel, lifecycle-observer terminal clears) doubling as the pre-departure teardown signal |
...
| D8 | Pre-departure "Seek anew" drives the live engine (boots at gateway) and pulses | ~~Pre-departure reroll regenerates the pending session's chain (`regeneratingRemainder(0, lastKnownLocation, …)`), silently — no engine, no pulse; in-walk reroll pulses (ece26a7)~~ **Superseded 2026-07-15** by the D3 revision: the engine is live pre-departure, so the reroll rides `engine.seekAnew` with the immediate stale-distance pulse on both platforms — divergence closed; `rerollPendingSession` deleted | Original rationale ("engine boots at walk start on Android") no longer holds |
