...
> **Android files:** `app/src/main/java/org/walktalkmeditate/pilgrim/data/collective/ContributionLedger.kt` (new), `data/collective/CollectiveRepository.kt` + `CollectiveCounterDelta.kt` (snapshot gains `walkUuid`) + `CollectiveQualifiers.kt`, `walk/WalkFinalizationObserver.kt` (call site passes the uuid — Android analogue of `ce7a750`'s `MainCoordinatorView` change), provider in `di/CollectiveModule.kt`; tests `data/collective/ContributionLedgerTest.kt` + extensions to `CollectiveRepositoryTest.kt` / `WalkFinalizationObserverTest.kt`.
