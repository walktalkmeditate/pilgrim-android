...
| D2 | `clock: org.walktalkmeditate.pilgrim.domain.Clock = Clock.System` parameter on the header (the house clock seam); iOS hardcodes `Date()` inside `refreshDailyLine()`. | The selection is date-keyed, so an uninjectable clock makes every render assertion flaky at UTC midnight (house determinism rule). Default preserves production behavior; only tests override. |
