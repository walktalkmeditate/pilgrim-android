...
- **`close` remediation status (updated 2026-05-16):**
  - **RESOLVED (code shipped + on-device verified):** setup.launch.loading (branded splash), setup.permissions.initial (iOS-verbatim copy + mic card + optional badges + button labels), settings.appearance 1/2/4 (iOS-verbatim copy + grouped card), settings.about (hero tree removed), settings.data-detail (export/import bare-action + journey chevrons), journal.inkscroll.populated (iOS-literal plurals).
