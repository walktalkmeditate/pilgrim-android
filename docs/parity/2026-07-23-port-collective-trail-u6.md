...
| D4 | `wasContributed` seeded once per VM (`buildState`); iOS re-reads on every `.onAppear`. | The ledger records at walk finalize (U4 L3), strictly before any summary for that walk can open, and the summary VM is per-visit — there is no window in which a re-read could change the answer mid-visit. |
